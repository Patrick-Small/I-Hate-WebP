/*
 * This program handles all functionality of the PNG Converter page in
 * the "I Hate WebP" Project. This program contains the ability to select
 * a PNG image from a file explorer, have that image appear on the screen,
...
PNG_converter::PNG_converter(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::PNG_converter)
{
    ui->setupUi(this);

    //Fill the drop down box
    ui -> convert_type -> addItem("WebP", QVariant::fromValue(ConvertTypePNG::WebP));
    ui -> convert_type -> addItem("JPEG", QVariant::fromValue(ConvertTypePNG::JPEG));

    //Connect the size text box to the labels
    connect(ui -> size, &QLineEdit::textChanged, this, &PNG_converter::updateSizeLabels);
...
void PNG_converter::on_select_file_clicked()
{
    //Type of filter
    ...
    QString path = QFileDialog::getOpenFileName(this, tr("Open PNG File"), prevDir, filter);
    ...
    if(!path.isEmpty())
    {
        ...
        QImage image;
        if (!image.load(path)) {
            QMessageBox::warning(this, tr("Error"), tr("Failed to load the PNG file."));
            return;
        }
...
void PNG_converter::on_Convert_clicked()
{
    //Gets the file path
    ...
    if (path.isEmpty())
    {
        ...
        QMessageBox::warning(this, tr("Error"), tr("Please select a PNG file."));
        return;
    }

    //Get the currently selected file extension
    ConvertTypePNG conversionType = ui -> convert_type -> currentData().value <ConvertTypePNG>();
...
void PNG_converter::convertFile(const QString &filePath, ConvertTypePNG conversionType)
{
    //Load a new image
    QImage image;
    ...
    case ConvertTypePNG::WebP:
        filename += ".webp";
        extension += "(*.webp)";
        break;
    case ConvertTypePNG::JPEG:
        filename += ".jpg";
        extension += "(*.jpg)";
        break;
    }
...
void PNG_converter::updateImageDisplay()
{
...
void PNG_converter::updateSizeLabels()
{
...
void PNG_converter::on_back_button_clicked()
{
    //Hide current page
    this -> hide();
...
void PNG_converter::closeEvent(QCloseEvent *event)
{
