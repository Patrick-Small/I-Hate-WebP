/*
 * This program handles all functionality of the WebP Converter page in
 * the "I Hate WebP" Project. This program contains the ability to select
 * a WebP image from a file explorer, have that image appear on the screen,
...
WebP_converter::WebP_converter(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::WebP_converter)
{
    ui -> setupUi(this);

    //Fill the drop down box
    ui -> convert_type -> addItem("PNG", QVariant::fromValue(ConvertTypeWebP::PNG));
    ui -> convert_type -> addItem("JPEG", QVariant::fromValue(ConvertTypeWebP::JPEG));

    //Connect the size text box to the labels
    connect(ui -> size, &QLineEdit::textChanged, this, &WebP_converter::updateSizeLabels);
...
void WebP_converter::on_select_file_clicked()
{
    //Type of filter
    QString filter = "WebP File (*.webp)";
    ...
    QString path = QFileDialog::getOpenFileName(this, tr("Open WebP File"), prevDir, filter);
    ...
    if(!path.isEmpty())
    {
        ...
        QImage image;
        if (!image.load(path)) {
            QMessageBox::warning(this, tr("Error"), tr("Failed to load the WebP file."));
            return;
        }
...
void WebP_converter::on_Convert_clicked()
{
    //Gets the file path
    ...
    if (path.isEmpty())
    {
        ...
        QMessageBox::warning(this, tr("Error"), tr("Please select a WebP file."));
        return;
    }

    //Get the currently selected file extension
    ConvertTypeWebP conversionType = ui -> convert_type -> currentData().value <ConvertTypeWebP>();
...
void WebP_converter::convertFile(const QString &filePath, ConvertTypeWebP conversionType)
{
    //Load a new image
    QImage image;
    ...
    case ConvertTypeWebP::PNG:
        filename += ".png";
        extension += "(*.png)";
        break;
    case ConvertTypeWebP::JPEG:
        filename += ".jpg";
        extension += "(*.jpg)";
        break;
    }
...
void WebP_converter::updateImageDisplay()
{
...
void WebP_converter::updateSizeLabels()
{
...
void WebP_converter::on_back_button_clicked()
{
    //Hide current page
    this -> hide();
...
void WebP_converter::closeEvent(QCloseEvent *event)
{
